...
void Chat::sendAllUserMesseges(int connection, char* i_message,std::shared_ptr<User> shpu) {      // SERVER
	...
	for (const auto& i : shpu->get_num_mess()) {
		...
		strcpy(i_message, getNameByLogin(message->getSender()).c_str());
		s_send(connection, i_message );
		bzero(i_message, MESSAGE_LENGTH);
		strcpy(i_message, message->getTime().c_str());
		s_send(connection, i_message);
		bzero(i_message, MESSAGE_LENGTH);
		strcpy(i_message, getNameByLogin(message->getRecipient()).c_str());
		s_send(connection, i_message);
		bzero(i_message, MESSAGE_LENGTH);
		strcpy(i_message, message->getText().c_str());
		s_send(connection, i_message);
		bzero(i_message, MESSAGE_LENGTH);	
